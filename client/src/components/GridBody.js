...
const GridBody = props => {
	...
	var rows = Products.forEach((row, index)=>{
		ProductDisplay.push(
    		...
    		<div className="card bg-light text-dark uni-card" >
		      	<div className="card-body">
			        <p style={{ fontSize:row.size,fontWeight:400 }} className="text-center card-title">{row.face}</p>
	    ...
	    if((index + 1) % 20 === 0){
	    	...
	    	<div className="card bg-light text-dark uni-card" >
		      	<div className="card-body">
			        <img src={`${API_BASE_PATH}/ads/?r=${Math.floor(Math.random()*1000)}`} style={{height:"180px", width:"100%", margin:"auto"}} className="img-responsive img-thumbnail"/>
		      	</div>
		    </div>
		    </div>
		    )
